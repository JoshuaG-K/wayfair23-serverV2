...
function lookAt(eye, target, up) {
    let zAxis = normalize(subtractVectors(eye, target)); // Forward
...
function startPosition(top_right_front, top_right_back, top_left_front) {
    const top_mid_front = midpoint(top_right_front, top_left_front);
    const forward = subtractVectors(top_right_front, top_left_front);
    return addVectors(top_mid_front, forward);
}
...
async function main() {
    ...
    console.log('Model bounding box path:', modelBoundingBoxPath);
    const response = await fetch(modelBoundingBoxPath);
    ...
    console.log("Initial view Matrix:", viewMatrix);
    console.log("Default view matrix: ", defaultViewMatrix);
    // Load splat
    const url = new URL(
        `download?splatPath=${encodeURIComponent(splatPath)}`,
       "http://osiris.cs.hmc.edu:15002/",//http://localhost:15002/",
        // this is where the URL of the file is loaded
    );
    ...
    const frame = (now) => {
        ...
        if (carousel) {
            ...
            const t = Math.sin((Date.now() - start_time) / 5000);
